...
pub fn resolve(frame: &RawFrame, edits: &Edits, profile: Option<&DcpProfile>) -> DcpSetup {
    let dcp_active = frame.meta.is_raw && edits.color.dcp.is_active() && profile.is_some();
    if let Some(profile) = profile.filter(|_| dcp_active) {
        let (matrix, resolved) =
            crate::ops::resolve_dcp(profile, frame.meta.wb_coeffs, &edits.color.dcp);
    ...
    let cam_to_srgb = if frame.meta.is_raw && !crate::color::is_unusable_matrix(&xyz_to_cam) {
        crate::color::cam_to_srgb_matrix(xyz_to_cam)
    } else {
        crate::color::identity_3x3()
    };
    let resolved = if frame.meta.is_raw && !edits.color.dcp.is_flat() {
