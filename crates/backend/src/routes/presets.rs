...
pub async fn list(State(state): State<AppState>) -> Result<Json<Vec<PresetRecord>>, AppError> {
    let presets = state.edits.list_presets().await?;
    Ok(Json(presets))
}
...
pub async fn create(
    ...
    let record = state
        .edits
        .create_preset(&parsed.name, parsed.group_name.as_deref(), &parsed.manifest)
        .await?;
    Ok(Json(record))
}
...
pub async fn delete(
    ...
    let deleted = state.edits.delete_preset(id).await?;
    if deleted {
