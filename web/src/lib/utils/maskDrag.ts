...
export type DragKind =
  ...
  | { kind: 'radial-feather' }
  | { kind: 'radial-rotate' }
  | { kind: 'polygon-vertex'; index: number; origin: Vec2f }
...
function snapToEighths(fixed: Vec2f, n: Vec2f, aspect: number): Vec2f {
  const dx = (n.x - fixed.x) * aspect;
  const dy = n.y - fixed.y;
  const len = Math.hypot(dx, dy);
  if (len < 1e-9) return n;
  const t = Math.round(Math.atan2(dy, dx) / SNAP_STEP) * SNAP_STEP;
  return { x: fixed.x + (len * Math.cos(t)) / aspect, y: fixed.y + len * Math.sin(t) };
}

function lockToAxis(origin: Vec2f, n: Vec2f, aspect: number): Vec2f {
  const dx = (n.x - origin.x) * aspect;
  const dy = n.y - origin.y;
  return Math.abs(dx) >= Math.abs(dy) ? { x: n.x, y: origin.y } : { x: origin.x, y: n.y };
}
...
function draggedRadial(
  ...
  if (drag.kind === 'radial-rx') {
    ...
    return { ...kind, radius_xy: { x: rx, y: shift ? rx * aspect : kind.radius_xy.y } };
  }
  if (drag.kind === 'radial-ry') {
    const ry = Math.max(0.005, Math.abs(across));
    return { ...kind, radius_xy: { x: shift ? ry / aspect : kind.radius_xy.x, y: ry } };
  }
...
export function draggedKind(
  ...
  drag: DragKind,
  n: Vec2f,
  ctx: DragContext = { aspect: 1, shift: false }
): MaskComponentKind | null {
  if (kind.kind === 'linear') {
    if (drag.kind === 'linear-p0') {
      return { ...kind, p0: ctx.shift ? snapToEighths(kind.p1, n, ctx.aspect) : n };
    }
    if (drag.kind === 'linear-p1') {
      return { ...kind, p1: ctx.shift ? snapToEighths(kind.p0, n, ctx.aspect) : n };
    }
  ...
  if (kind.kind === 'radial') return draggedRadial(kind, drag, n, ctx);

  if (kind.kind === 'polygon') {
    if (drag.kind === 'polygon-vertex') {
      const index = drag.index;
      const at = ctx.shift ? lockToAxis(drag.origin, n, ctx.aspect) : n;
      return { ...kind, points: kind.points.map((p, i) => (i === index ? at : p)) };
    }
