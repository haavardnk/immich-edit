<script lang="ts">
  ...
  function fetchPage(personId: string, initial: boolean): void {
    if (initial) {
      if (!loadedOnce) loading = true;
      nextPage = null;
      totalCount = undefined;
      searchStatistics(browseControls.statsBody({ personIds: [personId] }))
        .then((s) => (totalCount = s.total))
        .catch(() => {});
    }
    const body = browseControls.searchBody({ personIds: [personId] });
...
{:else}
  <BrowseHeader title="Person" loaded={assets.length} {totalCount} />
