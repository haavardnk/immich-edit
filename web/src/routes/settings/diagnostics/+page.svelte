...
{:else if health}
  ...
  <section class="space-y-2">
    ...
    <dl class="grid grid-cols-[160px_1fr] gap-y-1 text-xs">
      <dt class="text-immich-dark-fg/50">HEIC decode</dt><dd class={health.heif_codecs.hevc_decode ? 'text-emerald-400' : 'text-red-400'}>{codecLabel(health.heif_codecs.hevc_decode)}</dd>
      <dt class="text-immich-dark-fg/50">HEIC export</dt><dd class={health.heif_codecs.hevc_encode ? 'text-emerald-400' : 'text-red-400'}>{codecLabel(health.heif_codecs.hevc_encode)}</dd>
      <dt class="text-immich-dark-fg/50">AVIF decode</dt><dd class={health.heif_codecs.av1_decode ? 'text-emerald-400' : 'text-red-400'}>{codecLabel(health.heif_codecs.av1_decode)}</dd>
      <dt class="text-immich-dark-fg/50">AVIF export</dt><dd class={health.heif_codecs.av1_encode ? 'text-emerald-400' : 'text-red-400'}>{codecLabel(health.heif_codecs.av1_encode)}</dd>
    </dl>
  ...
  {#if timings}
    ...
    <section class="space-y-2">
      ...
      <dl class="grid grid-cols-[160px_1fr] gap-y-1 text-xs font-mono">
        <dt class="text-immich-dark-fg/50 font-sans">Preview frames</dt><dd>{formatBytes(timings.cache_bytes.preview_frames_used)} / {formatBytes(timings.cache_bytes.preview_frames_cap)}</dd>
        <dt class="text-immich-dark-fg/50 font-sans">Quality frames</dt><dd>{formatBytes(timings.cache_bytes.quality_frames_used)} / {formatBytes(timings.cache_bytes.quality_frames_cap)}</dd>
        <dt class="text-immich-dark-fg/50 font-sans">Mask rasters on disk</dt><dd>{formatBytes(timings.cache_bytes.rasters_disk_used)} / {formatBytes(timings.cache_bytes.rasters_disk_cap)}</dd>
      </dl>
    </section>
